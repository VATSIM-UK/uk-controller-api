...
class RecordFirEntry
{
    ...
    public function __construct()
    {
        $this->boundaries = FlightInformationRegion::all()
            ->mapWithKeys(function (FlightInformationRegion $region) {
                return [$region->id => FlightInformationRegionService::getBoundaryPolygon($region->identification_code)];
            })
            ->toArray();
    }
    ...
    public function handle(NetworkAircraftUpdatedEvent $event): bool
    {
        ...
        if (
            $lastEntry &&
            !$this->hasPreviouslyExitedFir($event->getAircraft()->callsign, $lastEntry->flight_information_region_id) &&
            isset($this->boundaries[$lastEntry->flight_information_region_id]) &&
            !$this->boundaries[$lastEntry->flight_information_region_id]->contains(new Coordinate($event->getAircraft()->latitude,
        ...
        foreach ($this->boundaries as $firId => $boundary) {
            ...
            if ($lastEntry && $firId === $lastEntry->flight_information_region_id) {
                $hasExited = NetworkAircraftFirEvent::where('callsign', $event->getAircraft()->callsign)
                    ->where('flight_information_region_id', $lastEntry->flight_information_region_id)
                    ->where('event_type', 'FIR_EXIT')
            ...
            if (
                isset($this->boundaries[$firId]) &&
                $this->boundaries[$firId]->contains(new Coordinate($event->getAircraft()->latitude,
                    $event->getAircraft()->longitude))
            ) {
                NetworkAircraftFirEvent::create(
                    [
